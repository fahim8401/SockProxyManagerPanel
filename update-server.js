...
function authenticateAdmin(req, res, next) {
    ...
    // For other admin routes, you might want to implement session/JWT authentication
...
<body>
    <div class="container">
        ...
        <div class="credentials">
            ...
            <p><strong>Endpoint:</strong> ${process.env.SERVER_IP || req.hostname}:1080</p>
        </div>
...
<body>
    ...
    <script>
        ...
        async function loadDashboard() {
            try {
                const [users, packages, connections] = await Promise.all([
                    apiCall('/api/users'),
                ...
                recentActivity.innerHTML = '<p>✅ System started successfully</p><p>✅ Database initialized</p><p>✅ Default users created</p>';
            } catch (error) {
        ...
        async function loadPackages() {
            try {
                ...
                // Update package dropdown in user modal
                    ...
                    <table>
                        ...
                        <tbody>
                            \${connections.map(conn => \`
                                <tr>
                                    <td>\${conn.user_id}</td>
                                    <td>\${conn.ip_address || 'Unknown'}</td>
                                    <td>\${conn.country || 'Unknown'}</td>
                                    <td>\${new Date(conn.connected_at * 1000).toLocaleString()}</td>
                                    <td>\${Math.round(conn.bytes_sent / 1024)} KB</td>
                                    <td>\${Math.round(conn.bytes_received / 1024)} KB</td>
                                    <td>
                                        <button class="btn btn-danger" onclick="disconnectUser(\${conn.id})">Disconnect</button>
                                    </td>
                                </tr>
                            \`).join('')}
                        </tbody>
        ...
        document.addEventListener('DOMContentLoaded', () => {
            loadDashboard();
            loadPackages(); // Load packages for user creation dropdown
        });
